...
class BluetoothSerialExample extends Component {
  ...
  componentWillMount () {
    ...
    BluetoothSerial.on('bluetoothEnabled', () => Toast.show('Bluetooth enabled', Toast.SHORT, Toast.BOTTOM))
    BluetoothSerial.on('bluetoothDisabled', () => Toast.show('Bluetooth disabled', Toast.SHORT, Toast.BOTTOM))
    BluetoothSerial.on('error', (err) => console.log(`Error: ${err.message}`))
    BluetoothSerial.on('connectionLost', () => {
      if (this.state.device) {
        Toast.show(`Connection to device ${this.state.device.name} has been lost`, Toast.SHORT, Toast.BOTTOM)
      }
  ...
  requestEnable () {
    ...
    .catch((err) => Toast.show(err.message, Toast.SHORT, Toast.BOTTOM))
  }
  ...
  pairDevice (device) {
    ...
    .then((paired) => {
      if (paired) {
        Toast.show(`Device ${device.name} paired successfully`, Toast.SHORT, Toast.BOTTOM)
        const devices = this.state.devices
        devices.push(device)
        this.setState({ devices, unpairedDevices: this.state.unpairedDevices.filter((d) => d.id !== device.id) })
      } else {
        Toast.show(`Device ${device.name} pairing failed`, Toast.SHORT, Toast.BOTTOM)
      }
    })
    .catch((err) => Toast.show(err.message, Toast.SHORT, Toast.BOTTOM))
  }
  ...
  write (message) {
    if (!this.state.connected) {
      Toast.show('You must connect to device first', Toast.SHORT, Toast.BOTTOM)
    }

    BluetoothSerial.write(message)
    .then((res) => {
      Toast.show('Successfuly wrote to device', Toast.SHORT, Toast.BOTTOM)
